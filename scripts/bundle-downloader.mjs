...
const getMainFilePath = async (appRootPath, redirectRulesPath) => {
  const redirectRules = JSON.parse(
    await fs.readFile(redirectRulesPath, 'utf-8')
  )
...
const injectVariableToBundle = (bundle, regExp) => {
  ...
  log(
    'Got function argument ' +
      '"' +
      chalk.bold(essential(functionArgument)) +
      '"'
  )
...
;(async function () {
  ...
  const mainFilePath = (
    await getMainFilePath(appRootPath, redirectRulesPath)
  ).replace('.js', '.bundle') // wait for fix https://github.com/webextension-toolbox/webextension-toolbox/issues/901
  ...
  const mainBundlePath = (await durakInstance('index.html')).body.match(
    bundleFileRegExp
  )[0]
  log(
    'Parsing bundle name successfully - ' +
      chalk.bold(succes(mainBundlePath)) +
      '\n'
  )
